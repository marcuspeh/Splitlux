...
from transaction.serializers import SimpleTransactionSerializer, TransactionCreateIncomingSerializer, TransactionSerializer, TransactionUpdateIncomingSerializer

# Create your views here.
class CreateTransaction(APIView):
    permission_classes = [IsUser]
    serializer_class = TransactionCreateIncomingSerializer
...
class UpdateTransaction(APIView):
    permission_classes = [IsUser]
    serializer_class = TransactionUpdateIncomingSerializer
    ...
    def patch(self, request, format=None, **kwargs):
        ...
        if serializer.is_valid():
            ...
            transaction.update(title=data['title'], amount=data['amount'], payers=payers, expenses=expenses)

            return Response(data=SimpleTransactionSerializer(transaction).data, status=status.HTTP_200_OK)
...
class DeleteTransaction(APIView):
    ...
    def delete(self, request, format=None, **kwargs):
        ...
        if user in group.members.all():
            ...
            return Response(data={"sucess": "Transaction deleted"}, status=status.HTTP_200_OK)
        else:
            return Response(data={"error": f"{user.name} is not in group"}, status=status.HTTP_400_BAD_REQUEST)